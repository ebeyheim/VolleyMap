...
} from "./chunk-ORMDWIXU.js";
import {
  DOCUMENT
} from "./chunk-KCRLH54Y.js";
import {
  APP_ID,
  ApplicationRef,
  CSP_NONCE,
...
var DragRef = class {
  ...
  _moveEvents = new import_rxjs5.Subject();
  ...
  _pointerMoveSubscription = import_rxjs5.Subscription.EMPTY;
  /** Subscription to the event that is dispatched when the user lifts their pointer. */
  _pointerUpSubscription = import_rxjs5.Subscription.EMPTY;
  /** Subscription to the viewport being scrolled. */
  _scrollSubscription = import_rxjs5.Subscription.EMPTY;
  ...
  beforeStarted = new import_rxjs5.Subject();
  /** Emits when the user starts dragging the item. */
  started = new import_rxjs5.Subject();
  /** Emits when the user has released a drag item, before any animations have started. */
  released = new import_rxjs5.Subject();
  /** Emits when the user stops dragging an item in the container. */
  ended = new import_rxjs5.Subject();
  /** Emits when the user has moved the item into a new container. */
  entered = new import_rxjs5.Subject();
  /** Emits when the user removes the item its container by dragging it into another container. */
  exited = new import_rxjs5.Subject();
  /** Emits when the user drops the item inside a container. */
  dropped = new import_rxjs5.Subject();
  /**
...
var DropListRef = class {
  ...
  sorted = new import_rxjs5.Subject();
  /** Emits when a dragging sequence is started in a list connected to the current one. */
  receivingStarted = new import_rxjs5.Subject();
  /** Emits when a dragging sequence is stopped from a list connected to the current one. */
  receivingStopped = new import_rxjs5.Subject();
  ...
  _viewportScrollSubscription = import_rxjs5.Subscription.EMPTY;
  ...
  _stopScrollTimers = new import_rxjs5.Subject();
  ...
  _startScrollInterval = () => {
    this._stopScrolling();
    (0, import_rxjs5.interval)(0, import_rxjs5.animationFrameScheduler).pipe((0, import_operators4.takeUntil)(this._stopScrollTimers)).subscribe(() => {
...
var DragDropRegistry = class _DragDropRegistry {
   ...
   */
  pointerMove = new import_rxjs5.Subject();
  /**
   * Emits the `touchend` or `mouseup` events that are dispatched
   * while the user is dragging a drag item instance.
   */
  pointerUp = new import_rxjs5.Subject();
  /**
   * Emits when the viewport has been scrolled while the user is dragging an item.
   * @deprecated To be turned into a private member. Use the `scrolled` method instead.
   * @breaking-change 13.0.0
   */
  scroll = new import_rxjs5.Subject();
  constructor() {
  }
  ...
  scrolled(shadowRoot) {
    ...
    if (shadowRoot && shadowRoot !== this._document) {
      streams.push(new import_rxjs5.Observable((observer) => {
        return this._ngZone.runOutsideAngular(() => {
    ...
    return (0, import_rxjs5.merge)(...streams);
  }
  /**
...
var CdkDrag = class _CdkDrag {
  ...
  _handles = new import_rxjs5.BehaviorSubject([]);
  _previewTemplate;
  _placeholderTemplate;
   ...
   * because this event will fire for every pixel that the user has dragged.
   */
  moved = new import_rxjs5.Observable((observer) => {
    const subscription = this._dragRef.moved.pipe((0, import_operators4.map)((movedEvent) => ({
      source: this,
  ...
  constructor() {
    ...
    if (dropContainer) {
      ...
      dropContainer._dropListRef.beforeStarted.pipe((0, import_operators4.takeUntil)(this._destroyed)).subscribe(() => {
  ...
  _syncInputs(ref) {
    ...
    ref.beforeStarted.pipe((0, import_operators4.take)(1)).subscribe(() => {
      if (this._parentDrag) {
  ...
  _setupHandlesListener() {
    this._handles.pipe(
      ...
      (0, import_operators4.tap)((handles) => {
      ...
      (0, import_operators4.switchMap)((handles) => {
        return (0, import_rxjs5.merge)(...handles.map((item) => item._stateChanges.pipe((0, import_operators4.startWith)(item))));
      }),
...
var CdkDropList = class _CdkDropList {
  ...
  _setupInputSyncSubscription(ref) {
    if (this._dir) {
      this._dir.change.pipe((0, import_operators4.startWith)(this._dir.value), (0, import_operators4.takeUntil)(this._destroyed)).subscribe((value) => ref.withDirection(value));
    }
    ref.beforeStarted.subscribe(() => {
  ...
  _handleEvents(ref) {
    ...
    (0, import_rxjs5.merge)(ref.receivingStarted, ref.receivingStopped).subscribe(() => this._changeDetectorRef.markForCheck());
  }
  /** Assigns the default input values based on a provided config object. */
  _assignDefaults(config) {
    const {
